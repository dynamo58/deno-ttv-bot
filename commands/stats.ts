...
const get_most_active_chatter_nickname = async (t: TwitchInfo, chatter_counts: Map<number, number>): Promise<[string, number]> => {
...
const Stats: CommandModule = {
	async execute(ctx: CommandContext): Promise<CommandResult> {
		...
		return await {
			is_success: true,
			output: `Uptime: ${hrs.toFixed(2)} hours | Games played: ${games} | Message count: ${lines} | Most active chatter: ${most_active_chatter[0]} with ${most_active_chatter[1]} lines Chatting`,
		}
	},
